...
void user_win::reinitialize(q_user* maker, std::map<std::string, q_user*> &master_list, std::map<std::string, std::vector<Tweet*> >& htags){

	main_user = maker;
	main_user->reset_feed();

	this->master_list = &master_list;
	this->all_htags = &htags;
...
{
	main_user->new_tweet((tweet_text->toPlainText()).toStdString(), *master_list, *all_htags);
